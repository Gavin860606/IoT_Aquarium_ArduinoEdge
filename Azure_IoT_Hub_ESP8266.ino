...
//這就是DirectMethod那邊有註冊的Callback Function，也就是回調給Server的那個Callback
void receivedDirectMethod(char* topic, byte* payload , unsigned int length)
{
  Serial.print("topic:");
  Serial.println(topic);
  //Method會有所不同，因此先從Topic中Filter事件名稱
  char* method = filter_methodName(topic);
  if(method != NULL){
    //Filter完畢後，交給他去handle不同Method的行為
...
//用於統一回覆Direct Method的MQTT publish
...
static void initializeClients()
{
  ...
  //這邊初始化MQTT client，設定要連線的MQTT Server config
  mqtt_client.setServer(host, port);
  //MQTT需要註冊callback function，用於回應Server端"有定義CB時"的回應，如果沒有這個callback, Direct method會timeout。
...
static int connectToAzureIoTHub()
{
  ...
  //MQTT 需要訂閱對應的topic，否則Client端連收都收不到，這是MQTT pub/sub的特性
  //這些Topic都會定義在Azure IoT Hub的source code，要自己去研究那些事幹嘛的
  
  //Topic的運作邏輯:
  //Server端發送MQTT過來，會帶該主題的Topic，Client端必須"事先"訂閱該主題才能收的到
  //並且需要針對該主題回應對應的response topic.... Server 端的Callback才收的到

  // C2D Topic是 Cloud-to-device，用於Azure發送訊息時的事件訂閱
  mqtt_client.subscribe(AZ_IOT_HUB_CLIENT_C2D_SUBSCRIBE_TOPIC);
  // 這個Topic就是 Direct Method的Topic.要去Source Code裡面研究才找的到....
...
{
  //判斷Telemetry topic是否符合AzureIoTHub裡面的格式
  ...
  telemetry_doc["pwm"] = (pwm_R +pwm_L) / 2;
...
void run(){
  ...
  Serial.println("℃");
  //判斷是否為AutoMode，如果前端那邊切成ManualMode時，就不需要套用他的邏輯控制風扇
  if(isAutoMode){
    autoMode();
  }
  //發送Telemetry
...
void autoMode(){
    // 溫度超過25度，打開風扇
  if( water_temperature >= 28.5 && water_temperature <29){
    pwm_R = 204;
    pwm_L = 204;
  ...
  if( water_temperature >= 28 && water_temperature <28.5){
    pwm_R = 153;
    pwm_L = 153;
    analogWrite(FanControl_R, pwm_R);
    delay(100);
  ...
  if( water_temperature >= 27.5 && water_temperature <28){
    pwm_R = 100;
    pwm_L = 100;
    analogWrite(FanControl_R, pwm_R);
    delay(100);
  ...
  if( water_temperature >= 26.8 && water_temperature < 27.5){
    pwm_R = 50;
    pwm_L = 50;
    analogWrite(FanControl_R, pwm_R);
    delay(100);
